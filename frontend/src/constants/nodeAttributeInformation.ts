...
export const nodeAttributeInformation = {
  ...
  모델이 한 번에 처리할 수 있는 **최대 토큰 길이**를 의미합니다.  
  입력이 이 길이를 초과하면 잘리거나 슬라이딩 윈도우(stride) 방식으로 나누어 처리해야 합니다.  
  ## 왜 중요한가?
  - 컨텍스트 길이가 짧으면 긴 문장을 끝까지 이해하지 못함.  
  - 길이가 길수록 더 많은 문맥 정보를 활용할 수 있음.  
  - 하지만 길이가 늘어나면 메모리 사용량과 연산량이 **선형적으로 증가**함.  
  ...
  - $N > ctxLength$: 초과분은 잘리거나 분할 처리 필요
    ...
    embDim: {
      ...
      description: `
  토큰을 표현하는 벡터의 **차원 수**를 의미합니다.  
  이 값은 모델의 **표현력**과 **계산 복잡도** 모두에 직접적으로 영향을 줍니다.  
  ...
  | LLaMA-2 13B | 5,120                         | 130억 파라미터 |
    ...
    posType: {
      ...
      description: String.raw`
  토큰 임베딩에는 **순서 정보가 포함되지 않기 때문에**, 위치 정보를 별도로 추가해야 합니다.  
  이때 사용하는 방법이 **Positional Embedding**이며, 대표적으로 절대, 상대, RoPE 방식이 있습니다.  
  ...
  | 방식              | 개념 | 특징 | 사용 예시 |
  |-------------------|------|------|-----------|
  | 절대 위치 (Absolute) | 각 위치 $i$에 고유한 벡터 $p_i$를 부여 | 단순하고 직관적, 하지만 **길이가 고정됨** | GPT-2 |
  ...
  ## 정리
  - 짧은 문장 처리 → **절대 위치**로 충분  
  - 긴 문맥 일반화 필요 → **상대 위치** 또는 **RoPE** 권장  
  - 최신 모델(예: LLaMA, GPT-4 계열)은 주로 **RoPE** 사용
    `,
    },
  },
  tokenEmbedding: {
    vocabSize: {
      ...
      description: String.raw`
  모델이 다룰 수 있는 **고유 토큰(token)의 개수**를 의미합니다.  
  토큰은 단어, subword, 혹은 문자 단위로 정의되며, **토큰화 방식**에 따라 크기가 달라집니다.  

  ---

  ## 왜 중요한가?
  - Vocabulary Size가 클수록 더 많은 단어/표현을 직접 다룰 수 있음.  
  - 하지만 크기가 커질수록 **임베딩 행렬 크기**와 **출력 Softmax 계산량**이 증가.  
  ...
  ## 정리
  - Vocabulary Size는 **토큰 표현력과 연산 효율성의 균형**을 맞추는 핵심 파라미터.  
  ...
  `,
    ...
    embDim: {
      title: 'Embedding Dimension Size',
      description: String.raw`
  토큰을 표현하는 **임베딩 벡터의 차원 수**입니다.  
  각 토큰은 $d$ 차원 공간의 벡터로 매핑되며, 이 $d$ 값이 곧 Embedding Dimension 입니다.  
  
  ---
  
  ## 왜 중요한가?
  - **차원이 클수록**: 더 많은 의미적/문법적 특징을 담을 수 있어 표현력이 풍부해짐.  
  - **차원이 작을수록**: 연산량과 메모리 사용량이 줄어 빠른 학습/추론 가능.  
  - 따라서 모델 설계에서 **표현력 ↔ 효율성**의 균형을 맞추는 핵심 요소입니다.  
  ...
  ## 정리
  - **embDim은 모델 크기와 성능을 직접적으로 결정**하는 핵심 값.  
  - 값이 커지면 성능 향상 가능하지만, **계산량과 파라미터 수도 함께 증가**.  
  ...
  대표적으로 **Batch Normalization (BN)**, **Layer Normalization (LN)**, **RMS Normalization (RMSNorm)**이 사용됩니다.  

  ---

  ## 왜 필요한가?
  - 딥러닝에서는 층이 깊어질수록 **분산(variance) 변화**와 **기울기 불안정** 문제가 발생.  
  - 정규화를 적용하면 층 출력이 일정한 범위를 유지해 **학습 안정성**과 **일반화 성능**이 개선됨.  
  ...
  - **RNN/Transformer** + 소/가변 배치 → **LayerNorm**  
  - 단순/속도 우선, 최신 트렌드 → **RMSNorm**  
  `,
    },
    eps: {
      title: 'Epsilon',
      description: String.raw`
  수치적 안정성을 위해 **분모에 더해지는 작은 상수**입니다.  
  ...
  ## 왜 필요한가?
  - 딥러닝 학습 중 **특정 입력 분산이 0 또는 매우 작아질 수 있음**.  
  - 이 경우 나눗셈에서 **0으로 나누기(division by zero) 오류** 발생.  
  ...
  `,
    ...
    inDim: {
      ...
      description: String.raw`
  정규화가 적용되는 **입력 벡터(텐서)의 차원 크기**를 의미합니다.  
  일반적으로 이는 **이전 레이어의 출력 차원**과 동일해야 합니다.  
  ...
  - 즉, $inDim$이 곧 정규화가 계산되는 **축의 크기**  
  
  ---
  
  ## 정리
  - inDim은 **정규화할 대상 벡터의 크기**.  
  - 반드시 이전 레이어 출력 차원과 동일해야 함.  
  - 보통 모델 전체에서 **embDim과 동일하게 설정**됨.
    `,
    },
  },
  feedForward: {
    actFunc: {
      ...
      description: String.raw` 
  Feed Forward Network(FFN)에서 **비선형성(non-linearity)**을 추가하는 함수입니다.  
  선형 변환만으로는 표현할 수 없는 복잡한 패턴을 학습하기 위해 사용됩니다.  

  ---

  ## 왜 필요한가?
  - 선형 변환만 있으면 여러 층을 쌓아도 결국 **하나의 선형 변환**과 동일.  
  - 활성화 함수를 넣어야만 **비선형 표현**이 가능해져 복잡한 언어 패턴을 학습할 수 있음.  
  ...
  ## 정리
  - FFN에서는 ReLU 대신 **GELU**가 사실상 표준.  
  - 최신 모델(LLaMA, PaLM 등)은 **SwiGLU**로 더 나은 성능 달성.  
  - 어떤 함수를 쓰느냐에 따라 **표현력, 수렴 속도, 성능**에 차이가 생김.
  `,
    },
    feedForwardType: {
      title: 'Feed Forward Type',
      description: String.raw`
  Feed Forward Network(FFN)의 **구조적 변형 방식**을 의미합니다.  
  ...
  **수식**:
  ...
  - 입력을 두 개의 선형 변환으로 분기
  - 하나는 **값(Value)**, 다른 하나는 **게이트(Gate, 시그모이드/활성화)** 역할
  ...
  **수식**:
  ...
  - 변형된 형태: **SwiGLU, GeGLU** (게이트에 Swish/GELU 적용)
  ...
  ## 정리
  - **Standard**: 단순하고 효율적, 중소형 모델에서 여전히 많이 사용  
  - **Gated (GLU/SwiGLU/GeGLU)**: 최신 대규모 모델의 기본 선택, 더 좋은 성능  
  - 연구/실험 단계에서는 두 방식을 비교해 **성능 ↔ 효율성**을 직접 확인하는 것이 유용
  `,
    },
    hiddenDim: {
      title: 'Hidden Dimension Size',
      description: String.raw`
  Feed Forward Network(FFN)의 **내부 확장 차원 크기**를 의미합니다.  
  입력 차원(embDim)에 비해 몇 배로 확장할지 결정하며,  
  대부분의 Transformer에서는 보통 **4 × embDim**으로 설정합니다.  
  ...
  - $W_1 \in \mathbb{R}^{d \times h}$: **hiddenDim = h**  
  ...
  ## 정리
  - hiddenDim은 **FFN의 내부 폭(width)**을 결정하는 값.  
  - 전통적으로 **4× embDim**이 표준이지만,  
  - 최근 모델(LLaMA 계열)은 효율성을 위해 **2.7×~3.5× 비율**도 사용.  
  - 성능 ↔ 자원 효율성을 맞추는 핵심 설계 파라미터.
    ...
    bias: {
      title: 'Bias Enabled',
      description: String.raw`
  Feed Forward Network(FFN)나 Linear Layer에서 **편향(bias) 항을 포함할지 여부**를 결정합니다.  
  편향은 입력이 모두 0일 때도 출력을 비선형적으로 이동시켜, 모델이 더 유연한 함수를 학습할 수 있게 합니다.  
  
  ---
  
  ## 왜 중요한가?
  - **Bias 포함**: 모델이 데이터 분포에 맞게 기준점을 이동시킬 수 있음.  
  - **Bias 제거**: 파라미터 수가 줄고, 계산량이 약간 감소.  
  ...
  dropout: {
    dropoutRate: {
      title: 'Dropout Rate',
      description: String.raw`
  신경망 학습 시 일부 뉴런(노드)을 **무작위로 비활성화(drop)** 하는 비율을 의미합니다.  
  과적합(overfitting)을 방지하고, 다양한 서브 네트워크를 학습하는 효과를 냅니다.  

  ---

  ## 왜 중요한가?
  - **과적합 방지**: 특정 뉴런이나 연결에 의존하지 않도록 만듦.  
  - **일반화 성능 향상**: 여러 경로를 학습해 새로운 데이터에도 강건함.  
  - **앙상블 효과**: 매 학습 스텝마다 다른 뉴런이 꺼져 여러 모델을 학습하는 효과.  
  ...
  linear: {
    outDim: {
      ...
      description: String.raw`
  Linear Layer(선형 변환)의 **출력 벡터 차원 크기**를 의미합니다.  
  이 값은 곧 **다음 레이어 입력 차원**과 연결되므로 반드시 호환되어야 합니다.  
  특히 마지막 Linear Layer에서는 **Vocabulary Size**와 동일하게 설정해 토큰 확률 분포를 산출합니다.  

  ---

  ## 왜 중요한가?
  - 레이어 간 **차원 불일치 오류**를 방지하기 위해 맞춰야 함.  
  - 마지막 Linear에서는 **출력 차원 = 어휘 집합 크기(Vocabulary Size)** 가 되어야 Softmax 확률 계산 가능.  
  ...
  ## 정리
  - **중간 Linear**: 차원 확장/축소 역할 (표현력 강화, 연산량 조절).  
  - **출력 Linear**: outDim = Vocabulary Size → Softmax로 확률 분포 산출.  
  - outDim은 **다음 레이어 입력과 반드시 일치**해야 안전하게 모델이 연결됨.
  `,
    },
    weightTying: {
      title: 'Weight Tying',
      description: String.raw`
  입력 임베딩(Embedding Layer)과 출력 Linear Layer의 **가중치 행렬을 공유(tying)** 하는 기법입니다.  
  즉, 두 레이어에서 별도의 행렬을 학습하지 않고, 하나의 행렬을 같이 사용합니다.  

  ---

  ## 왜 사용하는가?
  - **파라미터 수 감소**: 대규모 모델에서 수억 개 이상의 파라미터 절약.  
  - **일관성 유지**: 입력 임베딩과 출력 로짓 공간이 같은 의미 공간을 공유 → 학습 안정성 증가.  
  - **성능 향상**: 실험적으로 perplexity 개선 효과 보고됨 (Press & Wolf, 2017).  

  ---

  ## 수식으로 이해하기

  1. **Weight Tying 없는 경우**  
  ...
  ## 정리
  - Weight Tying은 **Embedding Layer와 Output Layer를 같은 행렬로 공유하는 방법**.  
  - 메모리 절약 + 성능 향상을 동시에 노릴 수 있어, **GPT 계열 등 대부분의 LLM에서 기본적으로 사용**됨.
  `,
    },
    bias: {
      title: 'Bias Enabled',
      description: String.raw`
  Linear Layer(선형 변환)에서 **편향(bias) 항을 포함할지 여부**를 결정합니다.  
  편향은 출력 값을 일정 방향으로 평행 이동시켜, 데이터 분포에 맞는 **유연한 표현**을 가능하게 합니다.  
  
  ---
  
  ## 왜 필요한가?
  - **Bias 포함**: 입력이 모두 0이어도, 출력이 0이 아닌 특정 값으로 이동 가능 → 모델 표현력 증가.  
  - **Bias 제거**: 파라미터 수가 줄어 메모리/계산 효율 ↑. 대규모 Transformer에서는 Residual + Normalization 구조 덕분에 bias가 거의 필요 없음.  
  ...
  ## 정리
  - **소규모/중규모 모델**: bias를 사용하는 것이 일반적.  
  - **대규모 LLM**: 대부분 bias를 제거해도 성능 손실이 거의 없어 효율을 위해 제외.  
  - 따라서 \`bias\`는 **모델 크기와 목적**에 따라 선택적으로 적용.
    `,
    },
  },
  mhAttention: {
    numHeads: {
      ...
      description: String.raw`
  Multi-Head Attention에서 **병렬로 분리해 학습하는 어텐션 헤드의 개수**를 의미합니다.  
  각 헤드는 서로 다른 서브 공간(subspace)에서 Query-Key-Value 연산을 수행하여, 입력의 다양한 관계를 학습합니다.  
  
  ---
  
  ## 왜 여러 개를 쓰는가?
  - **한 개의 어텐션(단일 헤드)** → 특정 패턴(예: 문맥적 유사성)만 학습  
  - **여러 개의 어텐션(멀티 헤드)** → 병렬적으로 서로 다른 관점(구문, 의미, 위치 관계 등)을 학습  
  ...
  - 일반적으로 **embDim / headDim ≈ 64**가 되도록 설정하는 것이 표준.
    ...
    dropoutRate: {
      ...
      description: String.raw`
  어텐션 연산에서 **가중치 행렬(Attention Weights)**에 적용되는 드롭아웃 비율을 의미합니다.  
  입력 토큰 간 유사도를 확률로 변환한 뒤, 일부 연결을 무작위로 끊어 과적합을 방지합니다.  
  
  ---
  
  ## 왜 필요한가?
  - 어텐션은 모든 토큰 간 관계를 계산하므로 **매우 강력한 표현력**을 가짐  
  - 그러나 데이터가 적거나 모델이 크면, 특정 패턴(예: 특정 위치)만 과도하게 학습 → 과적합 위험  
  - Dropout을 적용하면 다양한 연결을 학습하게 되어 **일반화 성능**이 향상됨  
  ...
  ## 정리
  - Attention Dropout은 **어텐션 가중치 행렬에 적용되는 드롭아웃**.  
  - 작은/중간 모델 → 0.1 정도 권장  
  ...
  - 값이 너무 크면 중요한 연결이 사라져 학습이 불안정해질 수 있음.
    ...
    qkvBias: {
      title: 'QKV Bias',
      description: String.raw`
  쿼리(Query), 키(Key), 값(Value) 벡터를 만들 때 사용하는 **선형 변환(Linear Layer)**에  
  편향(bias) 항을 포함할지 여부를 결정합니다.  
  
  - **Bias 포함**: 각 변환에 추가 자유도를 주어 작은·중간 규모 모델에서 표현력 향상 가능.  
  - **Bias 제거**: 최신 대규모 LLM에서는 성능 차이가 거의 없어, 파라미터 절감을 위해 제거하는 경우가 많음.  
  - 실제로 GPT-3, LLaMA-2/3 같은 모델은 대부분 QKV 변환에서 bias를 사용하지 않습니다.
    ...
    isRoPE: {
      title: 'RoPE Enabled',
      description: String.raw`
  **RoPE(Rotary Positional Embedding)** 적용 여부를 결정합니다.  
  
  - RoPE는 기존의 절대/상대 위치 임베딩 대신, **위치 정보를 벡터의 각 차원에 회전(rotary) 방식으로 인코딩**합니다.  
  ...
  **비사용 시**  
  - 전통적인 절대/상대 포지셔널 임베딩을 적용.  
    ...
    ropeBase: {
      title: 'Rope Base',
      description: String.raw`
  **RoPE(Rotary Positional Embedding)**에서 사용하는 **기본 각도(θ, theta) 값**을 정의합니다.  
  
  RoPE는 위치 정보를 각 차원별로 주기적인 회전 값으로 인코딩하는데,  
  이때 사용하는 주기의 기본값이 \`ropeBase\`입니다.  
  
  ---
  
  ## 개념적으로
  - 작은 \`ropeBase\`: 더 짧은 주기로 회전 → 짧은 문맥에 민감.  
  - 큰 \`ropeBase\`: 더 긴 주기로 회전 → 긴 문맥 정보 보존에 유리.  
  ...
  ## 정리
  - \`ropeBase\`는 모델이 **얼마나 긴 문맥까지 안정적으로 처리할 수 있는지**와 직결됩니다.  
  ...
  gqAttention: {
    numHeads: {
      title: 'Number of Heads',
      description: String.raw`
  어텐션을 **여러 개의 헤드(head)**로 나누어 병렬 계산할 때,  
  그 헤드의 개수를 정하는 값입니다.  
  
  - 헤드 수가 많을수록: 더 다양한 관점에서 문맥을 해석 가능 (정밀 ↑, 계산량 ↑).  
  - 헤드 수가 적을수록: 계산은 가벼워지지만, 표현력이 제한될 수 있음.  
  
  즉, \`numHeads\`는 **모델이 동시에 몇 개의 시선으로 문장을 바라보는지**를 결정합니다.
    `,
    },
    ...
    ctxLength: {
      title: 'Context Length',
      description: String.raw`
  모델이 한 번에 볼 수 있는 **최대 토큰 수**를 의미합니다.  
  
  - 이 값보다 긴 입력은 잘리거나 여러 조각으로 나눠 처리해야 합니다.  
  - 길이가 길수록 더 많은 문맥을 이해할 수 있지만, **메모리 사용량과 계산량이 크게 증가**합니다.  
  
  즉, \`ctxLength\`는 모델이 **한 번에 기억할 수 있는 문장의 최대 길이**를 정하는 값입니다.
    `,
    },
    ...
    dropoutRate: {
      title: 'Attention Dropout Rate',
      description: String.raw`
  어텐션 가중치에 적용되는 **드롭아웃 비율**입니다.  
  
  - 학습 과정에서 일부 연결을 무작위로 끊어, **과적합을 방지**합니다.  
  ...
  보통 0.1 ~ 0.3 범위에서 사용합니다.
    ...
    qkvBias: {
      title: 'QKV Bias',
      description: String.raw`
  쿼리(Q), 키(K), 값(V) 벡터를 만들 때 사용하는 **선형 변환(Linear Projection)**에  
  **편향(bias) 항을 포함할지 여부**를 결정합니다.  
  ...
  - $b_Q, b_K, b_V$는 선택적으로 추가되는 **Bias 벡터**  
  - \`qkvBias = true\` → 각 변환에 $b$를 포함  
  - \`qkvBias = false\` → 순수 선형 변환만 적용
  
  ---
  
  ## 왜 필요한가?
  - **Bias 포함 (true)**  
    - 각 Q/K/V가 **0 이외의 기준점(offset)**에서 시작 가능  
    - 작은 모델이나 데이터가 제한적인 경우, 표현력 향상에 도움  
  - **Bias 제외 (false)**  
    - 불필요한 파라미터를 줄여 **계산 효율성** 및 **훈련 안정성** 확보  
  ...
  ## 정리
  - \`qkvBias\`는 Q/K/V 선형 변환에 **bias 항을 포함할지 여부**  
  - 작은/중간 규모 모델 → \`true\` (표현력 ↑)  
  - 대규모 모델 (수십억 파라미터) → \`false\` (효율/안정성 ↑)  
  ...
  RoPE는 **위치 정보를 임베딩에 회전(Rotation) 연산으로 삽입**하는 방식입니다.  
  ...
  - RoPE (Su et al., 2021)  
    → **쿼리(Q), 키(K)** 벡터에 위치에 따른 **회전 행렬**을 적용  
  ...
  내적 $q'^T k'$ 계산에 **상대적 위치 차이**가 직접 반영됩니다.
  
  ---
  
  ## 장점
  - **상대적 위치 인코딩** 가능 → 길이에 무관하게 일반화 ↑  
  - **긴 문맥 처리**에 강함 (LLaMA, GPT-NeoX 등 활용)  
  - 추가 파라미터 없음 → 효율적  
  ...
  ## 정리
  - RoPE는 **위치 정보를 회전 연산으로 삽입하는 기법**  
  - 긴 문맥, 대규모 모델에서 사실상 표준  
  - 실험적/교육적 목적 아니면 대부분 \`true\`로 설정
    `,
    },
    ...
    ropeBase: {
      title: 'RoPE Base',
      description: String.raw`
  RoPE에서 사용하는 **기본 주파수 스케일 값(θ, theta)**을 정의합니다.  
  ...
  - $d_{model}$ : 임베딩 차원 크기  
  - \`ropeBase\`가 크면 → 각도 변화가 완만 → **긴 문맥 표현에 유리**  
  - \`ropeBase\`가 작으면 → 각도 변화가 급함 → **짧은 문맥에 집중**  
  
  ---
  
  ## 직관적으로
  - 작은 값 (예: 1,000) → 위치 차이가 빨리 커져서 **짧은 범위 관계에 민감**  
  - 큰 값 (예: 10,000, 100,000) → 위치 차이가 천천히 반영되어 **긴 범위 관계 유지**  
  ...
  ## 정리
  - \`ropeBase\`는 RoPE에서 **위치 회전 주파수의 기준값**  
  - 값이 클수록 모델은 **더 긴 문맥**을 안정적으로 다룸  
  ...
  transformerBlock: {
    numOfBlocks: {
      ...
      description: String.raw`
  트랜스포머 인코더/디코더 블록을 **몇 번 반복할지(=쌓을지)**를 지정하는 값입니다.  
  모델의 **깊이(depth)**와 **표현력**을 직접적으로 결정합니다.  
  
  ---
  
  ## 기본 개념
  - 하나의 Transformer Block =  
    **멀티헤드 어텐션(MHA)** + **피드포워드 네트워크(FFN)** + **정규화/잔차 연결**  
  - \`numOfBlocks\`는 이 블록을 **몇 층 쌓을지**를 결정  
  ...
  - 블록 수 ↑ →  
    - **장점**: 더 복잡한 패턴 학습, 성능 ↑  
    - **단점**: 연산량/메모리 사용량 급증, 과적합 위험 ↑  
  - 블록 수 ↓ →  
    - **장점**: 계산이 빠르고 가볍다  
    - **단점**: 복잡한 문맥 관계를 포착하기 어려움  
  ...
  dynamicBlock: {
    numOfBlocks: {
      title: 'Number of Blocks',
      description: String.raw`
  **동적 블록(Dynamic Block)** 안에 포함될 하위 블록의 수를 지정합니다.  
  여기서 각 블록은 동일하지 않고, **서로 다른 구성 요소(예: 어텐션, FFN, 정규화 등)를 독립적으로 설정**할 수 있습니다.  
  
  ---
  
  ## 기본 개념
  - \`transformerBlock\` = 동일한 구조의 블록을 여러 번 반복  
  - \`dynamicBlock\` = 각 블록이 독립적 → 서로 다른 하이퍼파라미터/구조 조합 가능  
  
  ---
  
  ## 왜 필요한가?
  - 모델의 특정 구간은 **어텐션 위주**, 다른 구간은 **피드포워드 위주**로 구성 가능  
  - 실험적으로 **구조 변형**을 적용해 성능 차이를 관찰할 수 있음  
  ...
  ## 정리
  - \`numOfBlocks\` = Dynamic Block 안에서 **몇 개의 독립 블록을 둘지** 결정  
  - 실험적/맞춤형 아키텍처 설계에 필수  
  ...
  testBlock: {
    testType: {
      title: 'Test Type',
      description: String.raw`
    **테스트 블록(Test Block)**의 타입을 지정합니다.  
    주로 모델 설계 과정에서 **새로운 모듈을 시험**하거나, **특정 구성 요소의 동작을 검증**하기 위해 사용됩니다.  
    
    ---
    
    ## 기본 개념
    - 일반 블록: 모델 학습/추론에서 실제 사용  
    - 테스트 블록: 실험적 목적 → 다양한 **testType**을 넣어 동작 확인  
    ...
    ## 예시 testType
    - \`"dummy"\ → 입력/출력을 그대로 통과시키는 블록  
    - \`linear\` → 단순 선형 변환만 적용  
    - \`attention_test\` → 어텐션 모듈만 별도로 시험  
    - \`ffn_test\` → 피드포워드 네트워크만 독립적으로 시험  
    
    ---
    
    ## 정리
    - \`testType\`은 **테스트 블록의 역할/구조**를 정의하는 핵심 값  
    - 다양한 타입을 통해 실험적 시나리오를 쉽게 구성 가능  
    - 실제 프로덕션 모델보다는 **연구/디버깅/비교 실험용**으로 활용
        `,
    },
  },
};
