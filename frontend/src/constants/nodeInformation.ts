export const nodeInformation = {
  ...
  ## Token Embedding이란?
  - 나눠진 토큰을 **숫자 벡터**로 바꾸는 과정입니다.  
  - 컴퓨터는 글자를 직접 이해하지 못하기 때문에, 토큰을 고정된 차원의 벡터로 변환합니다.  
  - 이 벡터는 단어의 **의미적 특징**을 담고 있어, 비슷한 의미의 단어는 가까운 위치에 놓이게 됩니다.  
  ...
  ## 동작 방식
  1. **토큰화**: 문장을 토큰 단위로 분해 → ["I", "love", "apple", "##s"]  
  2. **토큰 ID 변환**: 각 토큰에 숫자 ID 할당 → [101, 202, 305, 401]  
  3. **임베딩 행렬 lookup**: ID에 해당하는 벡터를 찾아옴 → [0.1, 0.2, …]  
  4. **결합**: 이 벡터에 위치 정보(Positional Embedding)를 더해 Transformer로 전달.  
  ...
  ## 왜 중요한가?
  - 텍스트를 **수치적 데이터**로 바꿔 모델이 이해할 수 있게 함.  
  - 단어 간 **의미적 유사성**을 반영하여 더 자연스러운 학습 가능.  
  - 드문 단어도 subword 단위로 나눠 처리할 수 있어 **유연성**이 높음.  
  
  ---
  
  ## 우리 프로그램에서
  - 사용자가 입력한 문장은 먼저 **토큰 단위**로 쪼개집니다.  
  - 각 토큰은 **숫자 벡터(Token Embedding)** 로 변환됩니다.  
  - 이 벡터는 위치 정보와 합쳐져 Transformer 블록으로 전달됩니다.  
  - 따라서 Token Embedding은 우리 모델의 **첫 출발점**이자, 텍스트 이해의 기본 단계입니다.
    `,
  },
  positionalEmbedding: {
  ...
  - 토큰 임베딩은 단어의 의미를 벡터로 바꾸지만, **순서 정보**는 담고 있지 않습니다.  
  - 따라서 문장에서 **"나는 밥을 먹었다"** 와 **"밥을 나는 먹었다"** 가 같은 벡터로 해석될 수 있습니다.  
  - 이를 막기 위해, 각 토큰에 **위치 정보**를 담은 벡터를 더해줍니다.  
  ...
  ## 절대 vs 상대 포지션
  - **절대 위치 임베딩**: "첫 번째, 두 번째…"처럼 위치 자체를 직접 반영  
  - **상대 위치 임베딩**: "이 단어와 저 단어가 얼마나 떨어져 있는가?"에 초점을 맞춤  
  - 최신 모델들은 상대 임베딩이나 **RoPE(Rotary Positional Embedding)** 같은 변형 방식을 많이 사용합니다.
  
  ---
  
  ## 비유로 이해하기
  - 단어가 **지도 위 점**이라면, 포지셔널 임베딩은 **좌표축 눈금** 같은 역할을 합니다.  
  - "서울은 부산의 위쪽" 같은 **위치 관계**를 알려주는 셈입니다.
  
  ---
  
  ## 우리 프로그램에서는?
  - Token Embedding으로 만든 벡터에 **Positional Embedding**을 더해 Transformer 블록에 입력합니다.  
  - 화면에서는 **"토큰 의미 + 위치 정보 = 최종 입력"** 구조를 직관적으로 확인할 수 있습니다.  
  - **핵심 파라미터**  
  ...
  - FFN은 FNN(Feedforward Neural Network, 다층 퍼셉트론)의 한 형태로,  
    **입력 → 선형 변환 → 활성화 → 선형 변환 → 출력** 구조를 가집니다.  
  ...
  ## 어떻게 동작할까?
  1. **입력**: Self-Attention에서 나온 출력 벡터($x$)를 받습니다.  
  2. **첫 번째 선형 변환 (Fully Connected Layer)** → 차원을 확장합니다.  
     - 보통 $4 \times embDim$ 크기까지 늘립니다.  
  3. **활성화 함수(Activation)** 적용 (예: GELU, ReLU, SwiGLU 등).  
  4. **두 번째 선형 변환** → 다시 원래 차원($embDim$)으로 축소합니다.  
  ...
  ## 왜 중요한가?
  - **비선형성 추가**: 단순한 선형 변환만 하지 않고 더 복잡한 패턴을 학습할 수 있음.  
  - **표현력 확장**: Attention이 단어 간 관계를 본다면, FFN은 그 결과를 더 풍부한 특징으로 바꿔줌.  
  - **독립 처리**: 각 토큰은 독립적으로 FFN을 거치므로, "자기 자신을 다듬는 과정"이라고 볼 수 있음.
  ...
  - 사용자는 **hiddenDim 비율**과 **activation 종류**를 바꿔가며 성능 차이를 직접 확인할 수 있습니다.
    `,
  },
  dropout: {
  ...
  ## 드롭아웃(Dropout)이란?
  - 학습 중 일부 뉴런(노드)을 **무작위로 꺼버리는 기법**입니다.
  - 이렇게 하면 모델이 특정 뉴런이나 연결에 **과도하게 의존하는 것(과적합)**을 막아줍니다.
  ...
  2. 남아 있는 뉴런의 값은 비율에 맞게 **스케일 조정** (합이 유지되도록)  
  3. 매 학습 스텝마다 다른 뉴런이 꺼지기 때문에, **여러 네트워크를 앙상블한 효과**가 생김  
  ...
  ## 왜 중요한가?
  - **과적합 방지**: 학습 데이터에만 특화되지 않고, 새로운 데이터에도 잘 작동하게 만듦  
  - **일반화 성능 향상**: 다양한 조합의 뉴런이 학습에 참여 → 더 튼튼한 모델  
  
  ---
  
  ## Transformer에서의 Dropout
  책에서도 설명하듯, GPT 같은 Transformer에서는 드롭아웃을 주로:
  - **어텐션 가중치** 계산 후 (Attention Weights)  
  - **Feed Forward 층 출력** 후  
  ...
  ## 우리 프로그램에서는?
  - 사용자가 조절할 수 있는 핵심 파라미터:
    - \`dropoutRate\`: 뉴런을 꺼버리는 확률 (0.0 ~ 0.5 범위 권장, 보통 0.1~0.3)
  - 예: \`dropoutRate: 0.1\` → 전체 뉴런의 약 10%를 무작위 비활성화  
  - 화면에서 설정 값을 바꿔가며 모델의 **학습 안정성**과 **성능 차이**를 체험할 수 있습니다.
    `,
  },
  linear: {
    title: 'Linear Output',
  ...
  - Transformer 블록들을 지난 후, 마지막 단계에서 **내부 벡터(embDim 크기)**를 **어휘 집합 크기(vocabSize)** 로 변환합니다.
  - 이렇게 나온 벡터는 각 토큰에 대해 "다음 단어가 될 확률 분포"를 만들기 위한 **logits**로 사용됩니다 .
  
  ---
  
  ## 왜 중요한가?
  - **출력 공간 연결**: 모델 내부 표현을 사람이 읽을 수 있는 단어(토큰) 공간으로 변환.  
  - **확률 예측**: Softmax를 통해 각 단어의 확률을 구해 텍스트를 생성.  
  - **학습 가능**: W, b는 학습 과정에서 지속적으로 업데이트되어 모델 성능에 직접적으로 기여.  
  ...
  - bias: 편향 사용 여부  
  - **Linear Output Layer = "토큰 의미 → 단어 확률" 변환기** 역할을 합니다.
    `,
  },
  mhAttention: {
  ...
  - **Self-Attention**은 단어들 사이의 관계를 찾는 메커니즘입니다.  
  - 하지만 하나의 "관점(헤드)"만 있으면 모든 패턴을 다 잡기 어렵습니다.  
  - MHA는 **여러 개의 어텐션 헤드**를 두어 다양한 관점에서 문맥을 해석하고, 이를 합칩니다.
  ...
  2. $Q, K, V$를 **numHeads** 개로 쪼개어 병렬로 어텐션 수행  
  3. 각 헤드별 어텐션 출력 $Z_1, Z_2, …, Z_h$를 얻음  
  4. 이들을 **연결(concatenate)** 후, 최종 선형 변환 $W_O$를 적용  
  ...
  ## 왜 중요한가?
  - **다양한 관점 확보**: 어떤 헤드는 문법 관계, 다른 헤드는 의미 관계를 잘 포착  
  - **복잡한 문맥 이해**: 긴 문장에서 멀리 떨어진 단어 간 연결을 동시에 학습 가능  
  - **Transformer 성능의 핵심**: 언어모델이 다양한 패턴을 이해하는 비결
  
  ---
  
  ## 비유로 이해하기
  - 한 사람이 글을 읽으면 한 가지 해석만 가능할 수 있습니다.  
  - 여러 사람이 동시에 각자 관점에서 읽고 의견을 합치는 것 = **멀티헤드 어텐션**.
  
  ---
  
  ## 우리 프로그램에서는?
  - 사용자가 설정할 수 있는 **핵심 파라미터**:
    - \`numHeads\`: 병렬로 사용할 어텐션 헤드 개수  
    - \`embDim\`: 임베딩 차원 (전체 벡터 크기)  
    - \`qkvBias\`: Query/Key/Value 변환 시 바이어스 사용 여부  
  ...
  - 한 블록은 크게 **멀티헤드 어텐션(MHA)** 과 **포지션별 피드포워드 네트워크(FFN)** 로 이루어져 있습니다.  
  - 각 부분은 **잔차 연결(Residual Connection)** 과 **LayerNorm/RMSNorm** 으로 감싸져 있어, 깊은 네트워크에서도 안정적으로 학습할 수 있습니다.  

  ---
  
  ## 블록 내부 구조
  1. **입력**: 임베딩 + 위치 정보가 합쳐진 토큰 벡터 $X$  
  2. **(정규화 + 잔차) → MHA**: 입력을 정규화 후, 멀티헤드 어텐션을 적용 → 출력 $Z$  
  3. **(정규화 + 잔차) → FFN**: $Z$를 다시 정규화 후, 피드포워드 네트워크 적용 → 최종 출력 $Y$  
  ...
  ## 왜 중요한가?
  - **문맥 이해**: MHA가 단어 간 관계를 학습.  
  - **표현력 강화**: FFN이 각 단어 표현을 더 풍부하게 다듬음.  
  - **안정성**: 정규화와 잔차 연결이 기울기 소실/폭발을 방지하고 깊은 학습 가능.  
  - **반복 구조**: Transformer 모델은 수십~수백 개의 블록을 쌓아 올려 강력한 표현력을 얻음.  

  ---
  
  ## 비유로 이해하기
  - **MHA**: 교실에서 여러 학생이 동시에 의견을 내며 토론하는 과정.  
  - **FFN**: 각 학생이 토론 결과를 자기 생각에 맞게 정리·발전시키는 과정.  
  - **잔차 + 정규화**: 토론이 산으로 가지 않도록 정리해주는 선생님 역할.  
  ...
  - 사용자가 조절할 수 있는 **핵심 파라미터**:
    - \`embDim\`: 각 벡터의 크기  
    - \`numHeads\`: 어텐션 헤드 개수  
    - \`numOfBlocks\`: 블록을 몇 층 쌓을지  
    - \`dropoutRate\`: 안정성을 위한 드롭아웃 비율  
    - \`normalization\`: LayerNorm / RMSNorm 선택  
  ...
  ## Dynamic Block이란?
  - **사용자가 직접 구성하는 블록 단위**입니다.  
  - 기존 Transformer Block(MHA + FFN + Norm + Residual)처럼 고정된 구조가 아니라,  
    **Attention / FFN / Normalization / Dropout 등 다양한 컴포넌트**를 원하는 순서대로 조합할 수 있습니다.  
  - 즉, 실험적으로 **새로운 아키텍처 변형을 시도**할 수 있게 하는 유연한 단위입니다.  

  ---

  ## 어떻게 동작할까?
  1. 사용자가 블록 안에 들어갈 레이어들을 선택 (예: Attention → FFN → Norm).  
  2. 선택된 레이어들이 하나의 **연속된 서브 네트워크**로 묶임.  
  ...
  ## 왜 중요한가?
  - **유연한 실험**: 기존 Transformer Block을 변형해 새로운 구조를 쉽게 시도 가능.  
  - **모듈화 학습**: 블록 단위로 성능을 비교하거나 조합을 바꿔볼 수 있음.  
  - **교육 목적**: SLM 플랫폼에서 “블록 쌓기”를 통해 Transformer 내부 작동 원리를 직관적으로 이해 가능.  
  ...
  ## 우리 프로그램에서는?
  - Dynamic Block은 **실험 모드**에서 특히 유용합니다.  
  - 사용자가 조절할 수 있는 핵심 요소:
    - \`layers[]\`: 어떤 레이어들을 포함할지 (MHA, FFN, Norm, Dropout 등).  
    - \`order\`: 레이어 적용 순서.  
    - \`params\`: 각 서브 레이어별 하이퍼파라미터.  
  - 이 블록을 활용해 기존 Transformer Block과 **성능 비교**를 직접 체험할 수 있습니다.
  `,
  },
  residual: {
  ...
  - 신경망이 깊어질수록 **기울기 소실/폭발** 문제가 발생할 수 있습니다.  
  - 이를 해결하기 위해, 입력 $x$를 변환 결과 $F(x)$에 그대로 더해주는 구조를 **Residual Connection**이라고 합니다.  
  ...
  ## 왜 중요한가?
  - **기울기 소실 방지**: 역전파 시 원래 입력 경로가 열려 있어 안정적 학습 가능.  
  - **빠른 수렴**: 깊은 네트워크도 쉽게 학습.  
  - **성능 향상**: 단순히 층을 쌓는 것보다 훨씬 좋은 결과.  
  - **Transformer, ResNet 공통 핵심 기술**.  

  ---

  ## 비유로 이해하기
  - 팀 프로젝트에서 **원래 보고서(x)** 가 있고, 팀원이 내용을 수정/추가해서 **개선된 보고서(F(x))**를 만듭니다.  
  ...
  ## 우리 프로그램에서는?
  - Residual은 **별도 레이어라기보다는 연결 방식**입니다.  
  - 하지만 시각적으로 표시하여, 사용자가 **“원래 입력 + 변환 결과”** 구조를 직관적으로 확인할 수 있습니다.  
  - 핵심 포인트:
    - 거의 모든 Transformer Block에서 기본 제공.  
    - 사용자 설정 파라미터는 없음 (내부적으로 자동 적용).  
  - 따라서 Residual은 **안정성과 성능의 숨은 조력자** 역할을 합니다.
  `,
  },
  testBlock: {
  ...
  ## Test Block이란?
  - **노드 테스트 및 디버깅**을 위한 특수한 블록입니다.  
  - 실제 학습 목적보다는, 다양한 노드 타입(Embedding, Attention, FFN 등)을 **실험적으로 조합**하고 **동작을 확인**하기 위해 사용됩니다.  
  ...
  - UI에서 **드래그 앤 드롭 / 시각화 / 파라미터 입력**이 정상적으로 처리되는지 확인할 때.  

  ---

  ## 특징
  - 내부 구조는 고정되어 있지 않고, **사용자가 원하는 레이어**를 자유롭게 삽입 가능.  
  - 주로 다음 요소 테스트에 활용:
    - TokenEmbedding / PositionalEmbedding  
    - Multi-Head Attention  
    - Feed Forward  
    - Normalization / Dropout / Residual  
  - 출력 결과는 학습 성능보다는 **연결 안정성**과 **시각화 검증**이 목적.  

  ---

  ## 비유로 이해하기
  - Transformer Block이 **완성된 레고 세트**라면,  
  - Test Block은 **여러 부품을 맞춰보는 조립 테스트 공간**입니다.  
  - 완성품이 아니라, 실험 중간에 구조를 확인하는 데 초점을 둡니다.  

  ---

  ## 우리 프로그램에서는?
  - Test Block은 **부모 노드** 역할을 합니다.  
  - 사용자가 다양한 자식 노드(Embedding, Attention, FFN 등)를 붙여서 **노드 간 연결 방식**을 시험할 수 있습니다.  
  - 주로 개발/디버깅 단계에서 활용되며,  
  ...
  - **멀티헤드 어텐션(MHA)**의 변형 기법 중 하나입니다.  
  - 일반 MHA에서는 Query, Key, Value의 헤드 수가 동일합니다.  
  - 하지만 GQA에서는 **Query 헤드 수를 크게 하고, Key/Value 헤드는 더 적게 공유**합니다.  
  - 이렇게 하면 **계산량과 메모리 사용량을 줄이면서도 성능은 유지**할 수 있습니다.  

  ---

  ## 동작 방식
  1. 입력 벡터 $X$를 $Q, K, V$로 변환.  
  2. $Q$는 **많은 수의 헤드**로 분할, $K, V$는 **더 적은 그룹**으로 분할.  
  ...
  ## 왜 중요한가?
  - **효율성**: Key/Value 저장 및 계산량을 줄임 → 긴 문맥 처리에 유리.  
  - **성능 유지**: Query는 충분히 세분화되어 다양한 관점을 확보.  
  - **최신 모델 적용**: LLaMA-2, LLaMA-3 등 최신 SLM/LLM에서 기본적으로 사용.  

  ---

  ## 비유로 이해하기
  - 일반 MHA: **교실에 학생(Q), 참고서(K), 정답지(V)가 모두 1:1로 매칭**.  
  ...
  ## 우리 프로그램에서는?
  - GQA는 **MHA의 확장 노드**로 선택할 수 있습니다.  
  - 사용자가 조절할 수 있는 핵심 파라미터:
    - \`numHeads\`: Query 헤드 개수  
    - \`numKvGroups\`: Key/Value 그룹 개수  
    - \`embDim\`: 임베딩 차원  
    - \`dropout\`: 어텐션 드롭아웃 비율  
  - \`numKvGroups < numHeads\` 조건을 지켜야 합니다.  
  - 실험을 통해 MHA 대비 **메모리/속도 효율** 차이를 확인할 수 있습니다.
  `,
  },
};
